<?php
	...
	class CaptainUtils {
		...
		// Returns an HTTPS URL for the user's avatar. The base `get_avatar()` method
		// offered by WordPress returns an HTML snippet. `get_avatar_url` takes care
		// of stripping the wrapping HTML code, and replacing the image protocol from
		// HTTP to HTTPS.
		// 
		...
		function get_avatar_url($user_id) {
			// Remove the wrapping HTML code from the `get_avatar` output
			preg_match("/src='(.*?)'/i", get_avatar($user_id), $matches);
			// Return the URL of the user's avatar, after replacing HTTP with HTTPS
